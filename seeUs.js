...
var z1 = 0; var z2 = 0; var z3 = 0; var z4 = 0; 
var feedBack = []
var feed
var empty
var same
...
var message = "Congrats, you have gotten the number, do you want to play again hit the start button" 
var seconds = 60
var minutes = 4
function _(x){
...
function timerr(el){
				...
				if(minutes == 0){	
				_("container").innerHTML = "Whoops, your time is up..."+`<a href="#" onclick="start()" style="text-decoration: none;">Start</a>`
...
function help(el){
				if (_("helped").innerHTML == "" ){
					...
					_("toggle").innerHTML = `<button  onclick="help('helped')">Need help?</button>`
					fadeOut(el)
				}
				}
...
function Unfinished(){
				...
				if((_("numbera").value == a)&&(_("numberb").value == b)&&(_("numberc").value == c)&&(_("numberd").value == d)){
						_("container").innerHTML=""
						_("timer").style.display = "none"
						Congrats()
	            }else{
						feed = feedBack.map(function(item){
...
function genrand(){
					while((a==b)||(a==c)||(a==d)||(b==a)||(b==c)||(b==d)||(c==a)||(c==b)||(c==d)||(d==a)||(d==b)||(d==c)){
...
function guess(el){
					...
					if((_("numbera").value =="") || (_("numberb").value =="") || (_("numberc").value =="") || (_("numberd").value =="")){ 
					    _("crossCheck").innerHTML = "please enter a number in all fields to continue"
					    fadeInOut(el)
						Unfinished()
						
					}else if((_("numbera").value ==_("numberb").value) || (_("numbera").value ==_("numberc").value)||(_("numbera").value ==_("numberd").value)||(_("numberb").value==_("numbera").value)||(_("numberb").value==_("numberc").value)||(_("numberb").value==_("numberd").value)||(_("numberc").value==_("numbera").value)||(_("numberc").value==_("numberb").value)||(_("numberc").value==_("numberd").value)||(_("numberd").value==_("numbera").value)||(_("numberd").value==_("numberb").value)||(_("numberd").value==_("numberc").value)){
						...
						if(parseInt(_("numbera").value, 10) == a){
							y1 = 1
						}else if((parseInt(_("numbera").value, 10) == b) || (parseInt(_("numbera").value, 10) == c) || (parseInt(_("numbera").value, 10) == d)){
							z1 = 1
						}
						...
						if(parseInt(_("numberb").value, 10) == b){
							y2 = 1
						}else if((parseInt(_("numberb").value, 10) == a) || (parseInt(_("numberb").value, 10) == c) || (parseInt(_("numberb").value, 10) == d)){
							z2 = 1
						}
						...
						if(parseInt(_("numberc").value, 10) == c){
							y3 = 1
						}else if((parseInt(_("numberc").value, 10) == a) || (parseInt(_("numberc").value, 10) == b) || (parseInt(_("numberc").value, 10) == d)){
							z3 = 1
						}
						...
						if(parseInt(_("numberd").value, 10) == d){
							y4 = 1
						}else if((parseInt(_("numberd").value, 10) == b) || (parseInt(_("numberd").value, 10) == c) || (parseInt(_("numberd").value, 10) == a)){
							z4 = 1
						}
						var dead =  y1+y2+y3+y4
						var wounded = z1+z2+z3+z4
						var outCome = "Previous guess is: " + _("numbera").value + _("numberb").value + _("numberc").value + _("numberd").value +" "+ dead + "dead, " + wounded  + "wounded "
							
						for (var i = feedBack.length; i >= 0; i--) {
							if(feedBack[i-1]==outCome){
								_("crossCheck").innerHTML = "please change the number to get another hint"
					         fadeInOut(el)
